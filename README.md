...
Pairs with **[Azathoth](https://github.com/Yrrrrrf/azathoth)** (AI/MCP layer) as a personal dev acceleration system.
...
Ensure `~/go/bin` is in your `$PATH`. On NixOS, add it via `home.sessionPath` in home-manager, or use the uv wrapper below.
...
This installs a `yog` shim via the Python wrapper that proxies to the compiled Go binary — no PATH surgery required.
...
- **Language:** Go — [Cobra](https://github.com/spf13/cobra) · [Viper](https://github.com/spf13/viper) · [Lip Gloss](https://github.com/charmbracelet/lipgloss) · [godotenv](https://github.com/joho/godotenv)
